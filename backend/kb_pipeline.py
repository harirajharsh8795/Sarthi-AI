...
def ingest_user_document_task(
    ...
    try:
        ...
        # Deterministically parse patient demographics & organ system category and prepend header
        full_doc_text = "\n".join(t for _, t in pages_text)
        try:
            from report_parser import extract_demographics_from_text, infer_organ_system_from_text
            demos = extract_demographics_from_text(full_doc_text)
            organ_category = infer_organ_system_from_text(full_doc_text)
            ...
            if organ_category:
                header_lines.append(f"Report Category / Organ System: {organ_category}")
            header_lines.append("---------------------------------")
            header_prefix = "\n".join(header_lines)
            if pages_text:
